...
impl Processor {
    ...
    fn proccess_exchange(
        ...
        //starting up an iterater for the accounts list
        let account_info_iter = &mut accounts.iter();
        //setting the first account as taker(reciever of transaction)
        let taker = next_account_info(account_info_iter)?;
        //checking if the taker is signer
        if !taker.is_signer {
            return Err(ProgramError::MissingRequiredSignature);
        }

        //setting the account that is sending tokens
        let takers_sending_token_account = next_account_info(account_info_iter)?;
        //setting the takers account that will receive tokens 
        let takers_token_to_receive_account = next_account_info(account_info_iter)?;
        //setting the account that will temporarily store the tokens to be sent
        let pdas_temp_token_account = next_account_info(account_info_iter)?;
        let pdas_temp_token_account_info =
            TokenAccount::unpack(&pdas_temp_token_account.try_borrow_data()?)?;
        let (pda, bump_seed) = Pubkey::find_program_address(&[b"escrow"], program_id);
        //checking if the amount that was expected is the same as the amount in the temporary token account
        if amount_expected_by_taker != pdas_temp_token_account_info.amount {
            return Err(EscrowError::ExpectedAmountMismatch.into());
        }
        //setting the initializers main account
        let initializers_main_account = next_account_info(account_info_iter)?;
        //setting the initializers token account that will recieve the tokens from taker
        let initializers_token_to_receive_account = next_account_info(account_info_iter)?;
        //setting the escrows account that is owned by the program
        let escrow_account = next_account_info(account_info_iter)?;
